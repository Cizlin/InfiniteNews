...
export async function getCustomizationImageUrl(folderDict, headers, title, waypointPath, mimeType, customizationCategory, customizationType, customizationCore = null, parentCustomizationType = null) {
	...
	if (customizationCategory != KeyConstants.CONSUMABLES_KEY && customizationCategory != KeyConstants.EMBLEM_PALETTE_KEY && customizationCategory != KeyConstants.MANUFACTURER_KEY) {
...
export async function getGeneralDictsAndArraysFromDbs(headers) {
	...
	await wixData.query(KeyConstants.SOURCE_TYPE_DB)
		.find()
		.then((results) => {
			if (results.items.length > 0) {
				for (let i = 0; i < results.items.length; ++i) {
	...
	while (retry && retryCount < maxRetries) {
		console.log("Fetching guide/xo JSON at " + WAYPOINT_URL_GUIDE);
		guideJson = await wixFetch.fetch(WAYPOINT_URL_GUIDE, {
				"method": "get",
				"headers": headers
			})
			.then( (httpResponse) => {
	...
	for (let i = 0; i < guideJson.Files.length; ++i) {
		...
		let extractedMatchArray = file.Uri.Path.match(/\d[^.]+/); // An array containing all matches in the Path string. Should be the Waypoint ID if it's valid.

		if (extractedMatchArray && extractedMatchArray.length > 0) {
			// Let's use the first match for now.
...
*/
// forceCheck: If this is true, then full comparison processing will be performed, even if the ETag matches (this is necessary for items with attachments since those relations aren't made in the item's JSON).
export async function getCustomizationItemToSave(folderDict, headers, customizationCategory, customizationDetails, generalDictsAndArrays, categorySpecificDictsAndArrays, forceCheck = false) {
	...
	else if (!(generalDictsAndArrays) || generalDictsAndArrays.length != 5) { // We expect 3 dicts/arrays in this construct.
	...
	if (customizationCategory == KeyConstants.ARMOR_KEY && customizationDetails.Attachments && customizationDetails.Attachments.length > 0) {
		let query = wixData.query(CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[KeyConstants.ARMOR_ATTACHMENT_KEY].CustomizationDb)
			.eq("waypointId", customizationDetails.Attachments[0])
		...
		for (let i = 1; i < customizationDetails.Attachments.length; i++) {
			query = query.or(wixData.query(CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[KeyConstants.ARMOR_ATTACHMENT_KEY].CustomizationDb)
				.eq("waypointId", customizationDetails.Attachments[i]));
		}
	...
	let kitItemIdArray = [];
	if ((customizationCategory in CUSTOMIZATION_TYPES_WITH_KITS) && CUSTOMIZATION_TYPES_WITH_KITS[customizationCategory].includes(siteCustomizationType) && customizationDetails.ChildItems 
		&& customizationDetails.ChildItems.length > 0) {

		let query = wixData.query(CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationDb)
			.eq("waypointId", customizationDetails.ChildItems[0])
		
		for (let i = 1; i < customizationDetails.ChildItems.length; i++) {
			query = query.or(wixData.query(CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationDb)
				.eq("waypointId", customizationDetails.ChildItems[i]));
		}
	...
	if ((customizationCategory in CUSTOMIZATION_TYPES_WITH_KITS) && (customizationCategory in CUSTOMIZATION_TYPES_WITH_ATTACHMENTS) && CUSTOMIZATION_TYPES_WITH_KITS[customizationCategory].includes(siteCustomizationType) && 
		customizationDetails.ChildAttachments && customizationDetails.ChildAttachments.length > 0) {

		let query = wixData.query(CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[KeyConstants.ARMOR_ATTACHMENT_KEY].CustomizationDb)
			.eq("waypointId", customizationDetails.ChildAttachments[0])
		
		for (let i = 1; i < customizationDetails.ChildAttachments.length; i++) {
			query = query.or(wixData.query(CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[KeyConstants.ARMOR_ATTACHMENT_KEY].CustomizationDb)
				.eq("waypointId", customizationDetails.ChildAttachments[i]));
		}
	...
	if ((customizationCategory in CUSTOMIZATION_TYPES_WITH_KITS) && CUSTOMIZATION_TYPES_WITH_KITS[customizationCategory].includes(siteCustomizationType) && (existingItem)) {
		let retry = true;
		let retryCount = 0;
		const maxRetries = 10;

		while (retry && retryCount < maxRetries) {	
			await wixData.queryReferenced(CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationDb, existingItem._id, CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationKitItemReferenceField)
				.then((results) => {
					retry = false;
		...
		existingItem[CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationKitItemReferenceField] = originalKitItemIds;

		if (customizationCategory in CUSTOMIZATION_TYPES_WITH_ATTACHMENTS) {
			let retry = true;
			let retryCount = 0;
			const maxRetries = 10;

			while (retry && retryCount < maxRetries) {	
				await wixData.queryReferenced(CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationDb, existingItem._id, CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationKitAttachmentReferenceField)
					.then((results) => {
						retry = false;
			...
			existingItem[CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationKitAttachmentReferenceField] = originalKitAttachmentIds;
		}
	}
	...
	if (existingItem && customizationDetails.EmblemPalettes && customizationDetails.EmblemPalettes.length > 0) {
		let retry = true;
		let retryCount = 0;
		const maxRetries = 10;

		while (retry && retryCount < maxRetries) {	
			await wixData.queryReferenced(CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationDb, existingItem._id, CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].EmblemPaletteReferenceField)
				.then((results) => {
					retry = false;
		...
		existingItem[CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].EmblemPaletteReferenceField] = originalEmblemPaletteIds;
	}
	...
	if (existingItem) {
		...
		if (customizationDetails.WaypointId in eTagDict && itemJson.itemETag != eTagDict[customizationDetails.WaypointId]) {
		...
		if (itemJson[CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationNameField] != customizationDetails.Title) {
			itemJson[CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationNameField] = customizationDetails.Title;

			changed = true;
			let returnedJsons = markItemAsChanged(itemJson, existingItem, CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationNameField);
			itemJson = returnedJsons[0];
			existingItem = returnedJsons[1];
		}

		if (customizationCategory != KeyConstants.ARMOR_ATTACHMENT_KEY) {
			if(itemJson[CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationSocketReferenceField] != customizationTypeId) {
				...
				changed = true;
				let returnedJsons = markItemAsChanged(itemJson, existingItem, CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationSocketReferenceField);
				itemJson = returnedJsons[0];
				existingItem = returnedJsons[1];
			}
		}
		...
		if ((customizationCategory in CUSTOMIZATION_TYPES_WITH_KITS) && CUSTOMIZATION_TYPES_WITH_KITS[customizationCategory].includes(siteCustomizationType) && !arrayCompare(originalKitItemIds, kitItemIdArray)) {
			itemJson[CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationKitItemReferenceField] = kitItemIdArray;

			// Update the needs review item and add a log to the changelog.
			changed = true;
			let returnedJsons = markItemAsChanged(itemJson, existingItem, CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationKitItemReferenceField);
			itemJson = returnedJsons[0];
			existingItem = returnedJsons[1];

			if ((customizationCategory in CUSTOMIZATION_TYPES_WITH_ATTACHMENTS) && !arrayCompare(originalKitAttachmentIds, kitAttachmentIdArray)) {
				itemJson[CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationKitAttachmentReferenceField] = kitAttachmentIdArray;

				// Update the needs review item and add a log to the changelog.
				changed = true;
				let returnedJsons = markItemAsChanged(itemJson, existingItem, CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationKitAttachmentReferenceField);
				itemJson = returnedJsons[0];
				existingItem = returnedJsons[1];
			}
		}
		...
		if (customizationDetails.EmblemPalettes && customizationDetails.EmblemPalettes.length > 0 && !arrayCompare(originalEmblemPaletteIds, customizationDetails.EmblemPalettes)) {
			itemJson[CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].EmblemPaletteReferenceField] = customizationDetails.EmblemPalettes;

			// Update the needs review item and add a log to the changelog.
			changed = true;
			let returnedJsons = markItemAsChanged(itemJson, existingItem, CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].EmblemPaletteReferenceField);
			itemJson = returnedJsons[0];
			existingItem = returnedJsons[1];
		}
		...
		// Might revisit this at some point, but now that we're automating the retrieval process for passes, capstones, shop, etc., we don't want this.
		...
		if (!customizationDetails.IsKitItem && existingItem.isKitItemOnly) {
			itemJson.isKitItemOnly = customizationDetails.IsKitItem; // This addition will allow the automation to convert an item from Kit Only to general use.

			changed = true;
			let returnedJsons = markItemAsChanged(itemJson, existingItem, "isKitItemOnly");
			itemJson = returnedJsons[0];
			existingItem = returnedJsons[1];
		}
		...
		if (!changed) { // If we didn't make any changes to the item, we can just skip it by returning 1.
			...
			itemJson.apiLastUpdatedDatetime = new Date();
		}
		...
		itemJson.waypointId = customizationDetails.WaypointId; // We're saving the Waypoint ID here.
		itemJson.itemETag = eTagDict[customizationDetails.WaypointId]; // We're save the current ETag here.
		itemJson.apiLastUpdatedDatetime = new Date();
		...
		if ((customizationCategory in CUSTOMIZATION_TYPES_WITH_KITS) && CUSTOMIZATION_TYPES_WITH_KITS[customizationCategory].includes(siteCustomizationType)) {
			itemJson[CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationKitItemReferenceField] = kitItemIdArray;
		...
		if (customizationDetails.EmblemPalettes && customizationDetails.EmblemPalettes.length > 0) {
			itemJson[CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].EmblemPaletteReferenceField] = customizationDetails.EmblemPalettes;
		}
		...
		itemJson.isKitItemOnly = customizationDetails.IsKitItem; // If we're adding a Kit item, we're going to treat it as Kit exclusive until the general version is reached.
		itemJson.sourceTypeReference = ((customizationDetails.IsKitItem) ? [sourceTypeDict["Kit Item"]] : [sourceTypeDict["(Pending)"]]); // This is a basic setup to let us link a new item with a default source type.
		itemJson.needsReview = true;
...
async function getCoreItemToSave(folderDict, headers, customizationCategory, customizationDetails, generalDictsAndArrays) {
	...
	else if (generalDictsAndArrays.length != 5) { // We expect 3 dicts/arrays in this construct.
		console.error("Unexpected length for generalDictsAndArrays. Expected 5, got ", generalDictsAndArrays.length);
	}
	...
	if (existingItem) {
		...
		if (itemJson[CORE_CATEGORY_SPECIFIC_VARS[customizationCategory].CoreNameField] != customizationDetails.Title) {
			itemJson[CORE_CATEGORY_SPECIFIC_VARS[customizationCategory].CoreNameField] = customizationDetails.Title;

			// Update the needs review item and add a log to the changelog.
			changed = true;
			let returnedJsons = markItemAsChanged(itemJson, existingItem, CORE_CATEGORY_SPECIFIC_VARS[customizationCategory].CoreNameField);
			itemJson = returnedJsons[0];
			existingItem = returnedJsons[1];
		}
...
export function getCustomizationDetailsFromWaypointJson(customizationCategory, waypointJson, waypointThemePathToCoreDict=null, attachmentArray=null, parentWaypointType=null, isKitItem=false, kitChildItemArray = [], 
		kitChildAttachmentArray = []) {
	...
	*/
	...
	itemJson.IsKitItem = isKitItem; // If this is true, this item was found as a child of a Kit.
	itemJson.ChildItems = kitChildItemArray;
	itemJson.ChildAttachments = kitChildAttachmentArray;
...
async function addEmblemPalettesToDb(emblemPalettePathArray, folderDict,headers) {
	...
	for (let i = 0; i < emblemPalettePathArray.length; ++i) {
		...
		// We're doing a simple active wait to ensure a configuration ID is only processed once at a time. This isn't perfect since the below check isn't atomic, but it should be just enough (I think...)
		...
		try {
			...
			while (retry && retryCount < maxRetries) {
				emblemPaletteDbId = await wixData.query(KeyConstants.EMBLEM_PALETTE_DB)
					.eq("configurationId", emblemPalettePathObject.ConfigurationId)
					.find()
					.then((results) => {
						retry = false;
			...
			if (!emblemPaletteDbId) {
				...
				let emblemPaletteUrl = await getCustomizationImageUrl(folderDict, headers, emblemPaletteName, emblemPaletteWaypointImageUrl, emblemPaletteMimeType, KeyConstants.EMBLEM_PALETTE_KEY, "Emblem Palette");
				...
				let emblemPaletteDbJson = {
					itemName: emblemPaletteName,
					image: emblemPaletteUrl,
					waypointId: emblemPaletteWaypointId,
					configurationId: emblemPaletteConfigurationId
				};
				...
				while (retry && retryCount < maxRetries) {
					emblemPaletteDbId = await wixData.insert(KeyConstants.EMBLEM_PALETTE_DB, emblemPaletteDbJson)
						.then((results) => {
							retry = false;
...
async function processItem(headers, customizationCategory, folderDict, generalDictsAndArrays, categorySpecificDictsAndArrays, itemWaypointPath, itemType, itemPathsProcessed, coreName = "", waypointThemePathToCoreDict = {}, 
	coreWaypointJsonArray = [], attachmentArray = [], attachmentPathToIdDict = {}, attachmentParentWaypointType = "", isKitItem = false, customizationWaypointIdArray = [], kitChildItemArray = [], 
	kitChildAttachmentArray = [], forceCheck = false) {

	// This helps us avoid processing duplicate items.
	if (itemPathsProcessed.includes(itemWaypointPath)) {
		if (itemType == ITEM_TYPES.attachment && !attachmentArray.includes(attachmentPathToIdDict[itemWaypointPath])) { // We still need to add the attachment Waypoint ID to the array if we haven't already.
			attachmentArray.push(attachmentPathToIdDict[itemWaypointPath]);
		}

		if (isKitItem) {
			let itemWaypointJson = await getCustomizationItem(headers, itemWaypointPath);
			customizationWaypointIdArray.push(itemWaypointJson.CommonData.Id);
		}
	...
	if (itemType == ITEM_TYPES.kit) {
		...
		attachmentArray.push(itemWaypointJson.CommonData.Id); // Add the ID to the array.
		attachmentPathToIdDict[itemWaypointPath] = itemWaypointJson.CommonData.Id; // Add the name to the dict as well.
	}
	...
	if (itemWaypointJson.CommonData.Type.includes("Emblem")) { // If the item is an emblem.
	...
	itemDetails = getCustomizationDetailsFromWaypointJson(customizationCategory, itemWaypointJson, waypointThemePathToCoreDict, attachmentArray, attachmentParentWaypointType, isKitItem, kitChildItemArray, 
		kitChildAttachmentArray);
	...
	if (itemType == ITEM_TYPES.core) {
		...
		itemSiteJson = await getCustomizationItemToSave(folderDict, headers, customizationCategory, itemDetails, generalDictsAndArrays, categorySpecificDictsAndArrays, forceCheck);
	}
...
// isKitItem: Only true if the item belongs to a Kit.
// customizationWaypointIdArray: Array of Waypoint IDs, only really needed for Kit items.
async function generateJsonsFromItemList(headers, customizationCategory, folderDict, generalDictsAndArrays, categorySpecificDictsAndArrays, customizationItemDbArray, customizationItemPathsProcessed, customizationItemPathArray, 
	waypointThemePathToCoreDict = {}, isKitItem = false, customizationWaypointIdArray = []) {
	
	for (let k = 0; k < customizationItemPathArray.length; ++k) {
		try {
			let itemPath = customizationItemPathArray[k];
			let itemDbJson = await processItem(headers, customizationCategory, folderDict, generalDictsAndArrays, categorySpecificDictsAndArrays, itemPath, ITEM_TYPES.item, 
				customizationItemPathsProcessed, "", waypointThemePathToCoreDict, [], [], {}, "", isKitItem, customizationWaypointIdArray);
...
async function generateJsonsFromItemAndAttachmentList(headers, customizationCategory, folderDict, generalDictsAndArrays, categorySpecificDictsAndArrays, customizationItemDbArray, customizationItemPathsProcessed, 
	itemAndAttachmentsArray, waypointThemePathToCoreDict = {}, isKitItem = false, customizationWaypointIdArray = [], customizationWaypointAttachmentIdArray = []) {
	...
	for (let l = 0; l < itemAndAttachmentsArray.length; ++l) {
		...
		for (let m = 0; m < itemAndAttachmentsArray[l].HelmetAttachments.OptionPaths.length; ++m) {
			try {
				...
				let attachmentDbJson = await processItem(headers, KeyConstants.ARMOR_ATTACHMENT_KEY, folderDict, generalDictsAndArrays, categorySpecificDictsAndArrays, attachmentPath, ITEM_TYPES.attachment, 
					customizationItemAttachmentPathsProcessed, "", waypointThemePathToCoreDict, [], attachmentArray, attachmentPathToIdDict, parentWaypointType, isKitItem, customizationWaypointAttachmentIdArray);
				...
				if (attachmentDbJson == 1) {
					...
					throw "Error occurred while getting the DB-ready item JSON for " + itemAndAttachmentsArray[l].HelmetAttachments.OptionPaths[m];
				}
			}
			catch (error) {
	...
	saveItemsToDbFromList(KeyConstants.ARMOR_ATTACHMENT_KEY, customizationItemAttachmentDbArray, ["ArmorAttachments"]);
	...
	for (let l = 0; l < itemAndAttachmentsArray.length; ++l) {
		try {
			...
			let itemDbJson = await processItem(headers, customizationCategory, folderDict, generalDictsAndArrays, categorySpecificDictsAndArrays, itemPath, ITEM_TYPES.item, 
				customizationItemPathsProcessed, "", waypointThemePathToCoreDict, [], parentPathToAttachmentArrayDict[itemPath], {}, "", isKitItem, customizationWaypointIdArray, [], 
				[], true); // We need to force all checks to occur in case the list of attachments changed (ETag might still match in this case).
			...
			if (itemDbJson == 1) {
				...
				throw "Error occurred while getting the DB-ready item JSON for " + itemAndAttachmentsArray[l].HelmetPath;
			}
		}
		catch (error) {
...
// waypointThemePathToCoreDict: A dictionary that takes theme Waypoint paths and converts them to core names.
async function generateJsonsFromThemeList(headers, customizationCategory, folderDict, generalDictsAndArrays, categorySpecificDictsAndArrays, customizationItemDbArray, customizationItemPathsProcessed, themePathArray, 
	waypointGroupsToProcess, coreName = "", waypointThemePathToCoreDict = {}) {
	...
	if (waypointGroupsToProcess.includes("Kits")) { // Skip the Kits if we aren't working on them.
			...
			if (themeWaypointJson.IsKit) { // Either we have a Kit or the default theme. If it's a kit, we just treat it like any other item.
				...
				for (let waypointTypeGroup in CUSTOMIZATION_WAYPOINT_GROUP_TYPES[customizationCategory]) {
					...
					if (!CUSTOMIZATION_WAYPOINT_GROUP_TYPES[customizationCategory][waypointTypeGroup]) { 
						...
						await generateJsonsFromItemList(headers, customizationCategory, folderDict, generalDictsAndArrays, categorySpecificDictsAndArrays, kitItemDbArray, kitItemPathsProcessed, 
							customizationItemPathArray, waypointThemePathToCoreDict, true, customizationIdArray);
					}
					else {
						let itemAndAttachmentsArray = themeWaypointJson[waypointTypeGroup]["Options"];
						await generateJsonsFromItemAndAttachmentList(headers, customizationCategory, folderDict, generalDictsAndArrays, categorySpecificDictsAndArrays, kitItemDbArray, 
							kitItemPathsProcessed, itemAndAttachmentsArray, waypointThemePathToCoreDict, true, customizationIdArray, customizationAttachmentsIdArray);
					}
				}
		...
		for (let l = 0; l < themePathArray.length; ++l) {
			...
			if (themeWaypointJson.IsKit) { // Either we have a Kit or the default theme. We only want to add Kits.
				try {
					let itemDbJson = await processItem(headers, customizationCategory, folderDict, generalDictsAndArrays, categorySpecificDictsAndArrays, themePathArray[l], ITEM_TYPES.kit, 
						customizationItemPathsProcessed, coreName, waypointThemePathToCoreDict, [], [], {}, "", false, [], kitPathToItemArrayDict[themePathArray[l]].items, kitPathToItemArrayDict[themePathArray[l]].attachments);
					if (itemDbJson == 1) {
						//console.log("Skipping " + itemPath);
						continue;
					}
		...
		if (!themeWaypointJson.IsKit) { // This is the default theme. We want to iterate over each of its sub items. These are defined in a constant.
			...
			for (let waypointTypeGroup in CUSTOMIZATION_WAYPOINT_GROUP_TYPES[customizationCategory]) {
				...
				if (!CUSTOMIZATION_WAYPOINT_GROUP_TYPES[customizationCategory][waypointTypeGroup]) { 
					...
					// We grab the array of waypoint paths, then process each one (so many nested for loops...)
					let customizationItemPathArray = themeWaypointJson[waypointTypeGroup].OptionPaths;
					await generateJsonsFromItemList(headers, customizationCategory, folderDict, generalDictsAndArrays, categorySpecificDictsAndArrays, customizationItemDbArray, customizationItemPathsProcessed, 
						customizationItemPathArray, waypointThemePathToCoreDict);
					...
					// Once we have that dictionary completed and all the attachment items added to their DB, we can proceed to work on the helmets.
					let itemAndAttachmentsArray = themeWaypointJson[waypointTypeGroup]["Options"];
					await generateJsonsFromItemAndAttachmentList(headers, customizationCategory, folderDict, generalDictsAndArrays, categorySpecificDictsAndArrays, customizationItemDbArray, 
						customizationItemPathsProcessed, itemAndAttachmentsArray, waypointThemePathToCoreDict);
				}
...
async function saveItemsToDbFromList(customizationCategory, customizationItemDbArray, waypointGroupsToProcess) {
	...
	let kitItemReferenceField = CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationKitItemReferenceField;
	let kitAttachmentReferenceField = CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationKitAttachmentReferenceField;
	let emblemPaletteReferenceField = CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].EmblemPaletteReferenceField;

	console.log("All items found for " + customizationCategory + " and groups", waypointGroupsToProcess, "Items:", customizationItemDbArray);
	...
	for (let i = 0; i < customizationItemDbArray.length; ++i) {
		...
		while (retryItem && retryCount < maxRetries) {
			await wixData.save(CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationDb, itemCopy, options)
				.then (async (item) => {
					retryItem = false;
					...
					if (HAS_CORE_ARRAY.includes(customizationCategory) && coreReferenceField in customizationItemDbJson) {
						...
						while (retry && retryCount < maxRetries) {
							await wixData.replaceReferences(CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationDb, coreReferenceField, item._id, customizationItemDbJson[coreReferenceField], options)
								.then(() => {
									retry = false;
								...
								.catch((error) => {
									console.error("Error ", error, " occurred. Try " + (++retryCount) + " of " + maxRetries + ". Was replacing core references for ", item._id, " in ", 
										CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationDb, " with ", customizationItemDbJson[coreReferenceField]);
								});
						}
					}
					...
					if ((customizationCategory in CUSTOMIZATION_TYPES_WITH_ATTACHMENTS) && attachmentReferenceField in customizationItemDbJson && customizationItemDbJson[attachmentReferenceField].length > 0) {
						...
						while (retry && retryCount < maxRetries) {
							await wixData.replaceReferences(CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationDb, attachmentReferenceField, item._id, customizationItemDbJson[attachmentReferenceField], options)
								.then(() => {
									retry = false;
								...
								.catch((error) => {
									console.error("Error ", error, " occurred. Try " + (++retryCount) + " of " + maxRetries + ". Was replacing attachment references for ", item._id, 
										" in ", KeyConstants.ARMOR_CUSTOMIZATION_DB, " with ", customizationItemDbJson[attachmentReferenceField]);
								});
						}
					}

					if ((customizationCategory in CUSTOMIZATION_TYPES_WITH_KITS) && kitItemReferenceField in customizationItemDbJson && customizationItemDbJson[kitItemReferenceField].length > 0) {
						...
						while (retry && retryCount < maxRetries) {
							await wixData.replaceReferences(CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationDb, kitItemReferenceField, item._id, customizationItemDbJson[kitItemReferenceField], options)
								.then(() => {
									retry = false;
								...
								.catch((error) => {
									console.error("Error ", error, " occurred. Try " + (++retryCount) + " of " + maxRetries + ". Was replacing Kit item references for ", customizationItemDbJson._id, " in ", 
										CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationDb, " with ", customizationItemDbJson[kitItemReferenceField]);
								});
						}

						if ((customizationCategory in CUSTOMIZATION_TYPES_WITH_ATTACHMENTS) && kitAttachmentReferenceField in customizationItemDbJson && customizationItemDbJson[kitAttachmentReferenceField].length > 0) {
							//console.log("Kit " + customizationItemDbJson[CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationNameField] + " has child attachments. Adding now.");
							let retry = true;
							let retryCount = 0;

							while (retry && retryCount < maxRetries) {
								await wixData.replaceReferences(CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationDb, kitAttachmentReferenceField, item._id, customizationItemDbJson[kitAttachmentReferenceField], options)
									.then(() => {
										retry = false;
										//console.log("Kit attachment references added for ", customizationItemDbJson[CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationNameField]);
									})
									.catch((error) => {
										console.error("Error ", error, " occurred. Try " + (++retryCount) + " of " + maxRetries + ". Was replacing Kit attachment references for ", customizationItemDbJson._id, 
											" in ", CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationDb, " with ", customizationItemDbJson[kitAttachmentReferenceField]);
									});
							}
						}
					}

					if ((customizationItemDbJson.sourceTypeReference && customizationItemDbJson.sourceTypeReference.length > 0)) { 
						// If the item has the source type reference field, it's a new item. We need to insert the references.
						//console.log("New item: " + customizationItemDbJson[CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationNameField] + " has sources ", customizationItemDbJson.sourceTypeReference);
						let retry = true;
						let retryCount = 0;

						while (retry && retryCount < maxRetries) {
							await wixData.insertReference(CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationDb, "sourceTypeReference", item._id, customizationItemDbJson.sourceTypeReference, options)
								.then(() => {
									retry = false;
								...
								.catch((error) => {
									console.error("Error ", error, " occurred. Try " + (++retryCount) + " of " + maxRetries + ". Was adding source type references for ", customizationItemDbJson._id, " in ", 
										CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationDb, " with ", customizationItemDbJson.sourceTypeReference);
								});
						}
					}

					if ((customizationItemDbJson[emblemPaletteReferenceField] && customizationItemDbJson[emblemPaletteReferenceField].length > 0)) { // If the item has the Emblem Palette reference field, we need to insert the references.
						...
						while (retry && retryCount < maxRetries) {
							await wixData.replaceReferences(CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationDb, emblemPaletteReferenceField, item._id, customizationItemDbJson[emblemPaletteReferenceField], options)
								.then(() => {
									retry = false;
								...
								.catch((error) => {
									console.error("Error ", error, " occurred. Try " + (++retryCount) + " of " + maxRetries + ". Was Emblem palette references for ", customizationItemDbJson._id, " in ", 
										CUSTOMIZATION_CATEGORY_SPECIFIC_VARS[customizationCategory].CustomizationDb, " with ", customizationItemDbJson[emblemPaletteReferenceField]);
								});
						}
					}
				})
	...
	if (HAS_CORE_ARRAY.includes(customizationCategory)) {
			...
			try{
				...
				for (let i = 0; i < coreList.length; i++) {
					try {
						...
						let coreSiteJson = await processItem(headers, customizationCategory, folderDict, generalDictsAndArrays, categorySpecificDictsAndArrays, coreList[i], ITEM_TYPES.core, corePathsProcessed, "", 
							waypointThemePathToCoreDict, coreWaypointJsonArray);
		...
		try{
			for (let i = 0; i < coreWaypointJsonArray.length; i++) {
				...
				await generateJsonsFromThemeList(headers, customizationCategory, folderDict, generalDictsAndArrays, categorySpecificDictsAndArrays, customizationItemDbArray, customizationItemPathsProcessed, themePathArray, 
					waypointGroupsToProcess, coreName, waypointThemePathToCoreDict);
			}
		...
		await generateJsonsFromItemList(headers, customizationCategory, folderDict, generalDictsAndArrays, categorySpecificDictsAndArrays, customizationItemDbArray, customizationItemPathsProcessed, customizationItemPathArray);
	}
...
export async function importManufacturers(headers) {
	...
	let metadataWaypointJson = await getCustomizationItem(headers, "Metadata/Metadata.json");
	let manufacturerArray = metadataWaypointJson.Manufacturers;

	let manufacturerDbArray = [];

	let manufacturerDbResults = await wixData.query(KeyConstants.MANUFACTURER_DB).find();
	...
	for (let i = 0; i < manufacturerArray.length; ++i) {
		if (manufacturerDbNameArray.includes(manufacturerArray[i].ManufacturerName)) { 
			...
			// To grab new info, delete the image from the file directory and delete the manufacturer from the DB (don't do this around 9:00 A.M. PST though.)
			continue;
		}

		let manufacturerDbJson = {
			manufacturer: manufacturerArray[i].ManufacturerName,
			image: await getCustomizationImageUrl(folderDict, headers, manufacturerArray[i].ManufacturerName, manufacturerArray[i].ManufacturerLogoImage, "image/png", KeyConstants.MANUFACTURER_KEY, null)
		}
	...
	if (manufacturerDbArray.length > 0) {
		await wixData.bulkInsert(KeyConstants.MANUFACTURER_DB, manufacturerDbArray)
		.then((results) => {
										...
										getCategorySpecificDictsAndArraysFromDbs(customizationCategoryArmor)
											.then((categorySpecificDictsAndArrays) => {
												updateDbsFromApi(headers, customizationCategoryArmor, ["Kits"], generalDictsAndArrays, categorySpecificDictsAndArrays)
													...
													.catch(error => {
														console.error("Failed to add Armor Kits due to " + error);
													});
											});
									}
									else {
...
export function spartanIdImport() {
	makeWaypointHeaders()
		.then((headers) => { // Getting the headers once and then using them a bunch is way more efficient than getting them for each request.
			importManufacturers(headers) // We're going to import the manufacturers before anything else. This isn't a super lengthy process, but will take a few seconds.
				.then(() => {
